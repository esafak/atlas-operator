...
ATLAS_RELEASE ?= dev
ATLAS_COMMIT ?= 3c09b47a906a3a92a00c43599555bc4a3590dd97
ATLAS_ASSET_BASE ?= https://github.com/$(ATLAS_REPOSITORY)/releases/download/$(ATLAS_RELEASE)
ATLAS_AMD64_SHA256 ?= 1f2a3832b895f2c78d32ba3bd38b2f94eab087d2756773813c4b31f89c295a4d
ATLAS_ARM64_SHA256 ?= bb70f066e86e9c030df71fc92853e9b54a494e2acf781f1740029c7834ed8edb
...
manifests: controller-gen ## Generate WebhookConfiguration, ClusterRole and CustomResourceDefinition objects.
	$(CONTROLLER_GEN) \
		...
		rbac:headerFile="hack/boilerplate.yaml.txt",roleName=manager-role \
...
test-e2e: ## Run the e2e tests. Expected an isolated environment using Kind.
	SKAFFOLD_PROFILE=${SKAFFOLD_PROFILE} go test -timeout 30m \
		-parallel $(shell nproc) \
		-v ./test/e2e/ \
		-run="^TestOperator/${TEST_RUN}"
...
install: manifests kustomize ## Install CRDs into the K8s cluster specified in ~/.kube/config.
	$(KUSTOMIZE) build config/crd | $(KUBECTL) apply -f - --server-side=true
...
addlicense: $(ADDLICENSE) ## Download golangci-lint locally if necessary.
$(ADDLICENSE): $(LOCALBIN)
	$(call go-install-tool,$(ADDLICENSE),github.com/google/addlicense,$(ADDLICENSE_VERSION))

.PHONY: license
license: addlicense ## Add license headers to all files.
