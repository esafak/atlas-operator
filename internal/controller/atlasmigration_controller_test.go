...
func TestMigration_ConfigMap(t *testing.T) {
	...
	assert(ctrl.Result{RequeueAfter: retryDuration}, false, "Migrating", `"SYNTAX ERROR" from version "20230808140359"`, "20230808132722")
	...
	require.Equal(t, []string{
		...
		`Warning TransientErr sql/migrate: executing statement "SYNTAX ERROR" from version "20230808140359": near "SYNTAX": syntax error`,
	}, h.events())
}
...
func TestMigration_MigrateDown_Remote_Protected(t *testing.T) {
	...
	}, false, "ProtectedFlowError", "migrate down is not allowed, set `migrateDown.allow` to true to allow downgrade", "", "", "")
	...
	assert(ctrl.Result{
		RequeueAfter: 2 * retryDuration,
	}, false, "ProtectedFlowError", "autoApprove is not allowed for a remote directory", "", "", "")
	...
	// Reconcile again
	assert(ctrl.Result{RequeueAfter: retryDuration}, false, "ApprovalPending", "plan approval pending, review here: THIS_IS_DEPLOYMENT_URL", "", "THIS_IS_DEPLOYMENT_URL", "")
	assert(ctrl.Result{RequeueAfter: retryDuration}, false, "ApprovalPending", "plan approval pending, review here: THIS_IS_DEPLOYMENT_URL", "", "THIS_IS_DEPLOYMENT_URL", "")
...
`,
	...
	assert(ctrl.Result{
		RequeueAfter: 2 * retryDuration,
	}, false, "ProtectedFlowError", "allow cannot be true without autoApprove for local migration directory", "", "", "")
...
func TestReconcile_BadSQL(t *testing.T) {
	...
	result, err = tt.r.Reconcile(context.Background(), migrationReq())
	require.NoError(tt, err)
	require.EqualValues(tt, reconcile.Result{
		RequeueAfter: retryDuration,
	}, result)
...
func TestReconcile_Transient(t *testing.T) {
	...
	result, err := tt.r.Reconcile(context.Background(), migrationReq())
	require.NoError(t, err)
	require.EqualValues(t, reconcile.Result{RequeueAfter: retryDuration}, result)
	require.Equal(t, []string{
