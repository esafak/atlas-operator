...
func TestReconcile_Reconcile(t *testing.T) {
	...
	assert(ctrl.Result{RequeueAfter: retryDuration}, false, "ReadSchema", "no target database defined")
	...
	}, false, dbv1alpha1.ReasonCreatingWorkingDir, "the desired state is not set")
	// Add schema,
	h.patch(t, &dbv1alpha1.AtlasSchema{
		ObjectMeta: meta,
		Spec: dbv1alpha1.AtlasSchemaSpec{
	...
	require.Equal(t, []string{
		...
		"Warning TransientErr the desired state is not set",
...
func TestReconcile_Credentials_BadPassSecret(t *testing.T) {
	...
	resp, err := tt.r.Reconcile(context.Background(), request)
	require.NoError(t, err)
	require.EqualValues(t, ctrl.Result{RequeueAfter: retryDuration}, resp)
	events := tt.events()
...
func TestConfigMapNotFound(t *testing.T) {
	...
	require.NoError(t, err)
	require.EqualValues(t, ctrl.Result{RequeueAfter: retryDuration}, res)
	cond := tt.cond()
